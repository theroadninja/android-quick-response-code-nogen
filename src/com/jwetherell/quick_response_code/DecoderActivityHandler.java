...
public final class DecoderActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case Configurations.auto_focus:
                // Log.d(TAG, "Got auto-focus message");
                // When one auto focus pass finishes, start another. This is the
                // closest thing to
                // continuous AF. It does seem to hunt a bit, but I'm not sure
                // what else to do.
                if (state == State.PREVIEW) cameraManager.requestAutoFocus(this, Configurations.auto_focus);
                break;
            case Configurations.restart_preview:
                Log.d(TAG, "Got restart preview message");
                restartPreviewAndDecode();
                break;
            case Configurations.decode_succeeded:
                ...
                break;
            case Configurations.decode_failed:
                // We're decoding as fast as possible, so when one decode fails,
                // start another.
                state = State.PREVIEW;
                cameraManager.requestPreviewFrame(decodeThread.getHandler(), Configurations.decode);
                break;
            case Configurations.return_scan_result:
    ...
    public void quitSynchronously() {
        ...
        Message quit = Message.obtain(decodeThread.getHandler(), Configurations.quit);
        quit.sendToTarget();
        try {
        ...
        removeMessages(Configurations.decode_succeeded);
        removeMessages(Configurations.decode_failed);
    }
    ...
    void restartPreviewAndDecode() {
        if (state == State.SUCCESS) {
            state = State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(), Configurations.decode);
            cameraManager.requestAutoFocus(this, Configurations.auto_focus);
